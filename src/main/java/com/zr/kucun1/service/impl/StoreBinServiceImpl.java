package com.zr.kucun1.service.impl;
...
/*
        ...
        try {
            ...
            for (int i = sheet.getFirstRowNum() + 4; i <= sheet.getLastRowNum(); i++) {
                ...
                if(xssfRow!=null) {
                    ...
                    System.out.println(String.valueOf(xssfRow.getCell(0)));//()内内容转为字符串形式
                    System.out.println(String.valueOf(xssfRow.getCell(1)));
                    System.out.println(String.valueOf(xssfRow.getCell(2)));
                    if (!StringUtils.isEmpty(String.valueOf(xssfRow.getCell(0)).trim()) &&//验证数据是否为空，转为字符串去掉两端空格
                            String.valueOf(xssfRow.getCell(0))!=null &&//数据去掉为null的情况
    ...
    private CustomerInfo build(XSSFRow xssfRow) throws Exception {
        ...
        addVO.setCustomerId(String.valueOf(xssfRow.getCell(0)));//前端要求Excel为文本格式,后台用string接收
